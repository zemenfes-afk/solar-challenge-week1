...
* A GitHub repository (`solar-challenge-week1`) was created and cloned.
* A Python virtual environment (`venv`) was set up.
* A `.gitignore` file was configured to ignore data, environment files, and CSVs.
* A `requirements.txt` file was generated with all necessary libraries (pandas, streamlit, seaborn, etc.).
...
4.  **Export:** The cleaned DataFrame for each country will be exported to a `_clean.csv` file (e.g., `benin_clean.csv`) in the `data/` directory.
